      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 bg-white border-r border-slate-200 flex flex-col flex-shrink-0">
            ...
            <div className="px-3 pb-2 flex gap-1">
              {([["likes", "인기"], ["recent", "최신"]] as const).map(([val, label]) => (
                <button
                  ...
                  className={`flex-1 text-xs py-1 rounded-lg transition-colors ${discoverSort === val ? "bg-emerald-50 text-emerald-700 font-medium" : "text-slate-500 hover:bg-slate-50"}`}
                >
                  {label}
                </button>
              ))}
          ...
          <div className="flex-1 overflow-y-auto px-2 pb-3 space-y-0.5">
            {sidebarTab === "discover" && filteredDiscover.map(p => (
              <UserPersonaCard
                key={p.id}
                persona={p}
                isSelected={selectedUserPersona?.id === p.id}
                isMine={p.creatorId === (user as any)?.userId}
        ...
        <main className="flex-1 overflow-y-auto">
          {!selectedUserPersona && !selectedMbtiPersona ? (
              ...
              {discoverPersonas.length > 0 && (
                <div>
                  <h2 className="text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                    <Compass className="w-4 h-4 text-emerald-500" />인기 페르소나
                  </h2>
                  <div className="grid grid-cols-2 gap-3">
                    {discoverPersonas.slice(0, 6).map(p => (
                      <button
                        key={p.id}
