...
대화 설정 (난이도 ${request.difficulty || 3} 기준 권장 범위):
${(() => {
  const d = request.difficulty || 3;
  if (d === 1) return '  - targetDurationMinutes: 5~8분, targetTurns: 6~8턴, minValidTurns: 3턴';
  if (d === 2) return '  - targetDurationMinutes: 7~10분, targetTurns: 8~12턴, minValidTurns: 4턴';
  if (d === 3) return '  - targetDurationMinutes: 10~15분, targetTurns: 12~16턴, minValidTurns: 5턴';
  return '  - targetDurationMinutes: 12~20분, targetTurns: 14~20턴, minValidTurns: 6턴';
})()}
...

{
  ...
  "minValidTurns": (targetTurns의 30~50% 수준, 최소 완료로 인정할 턴 수)
}
...
- ENTJ: 결단력 있고 목표 지향적, 효율성과 결과 중시, 리더십 발휘`;
  ...
  try {
    ...
    // Clamp generated values to valid per-difficulty ranges
    const difficultyLevel = data.difficulty || request.difficulty || 3;
    const difficultyRanges: Record<number, { durationMin: number; durationMax: number; turnsMin: number; turnsMax: number; minValid: number }> = {
      1: { durationMin: 5, durationMax: 8,  turnsMin: 6,  turnsMax: 8,  minValid: 3 },
      2: { durationMin: 7, durationMax: 10, turnsMin: 8,  turnsMax: 12, minValid: 4 },
      3: { durationMin: 10, durationMax: 15, turnsMin: 12, turnsMax: 16, minValid: 5 },
      4: { durationMin: 12, durationMax: 20, turnsMin: 14, turnsMax: 20, minValid: 6 },
    };
    const rec = difficultyRanges[difficultyLevel as 1|2|3|4] || difficultyRanges[3];

    const rawDuration = typeof data.targetDurationMinutes === 'number' ? data.targetDurationMinutes : rec.durationMin;
    const rawTurns    = typeof data.targetTurns === 'number'           ? data.targetTurns           : rec.turnsMin;
    const rawMinValid = typeof data.minValidTurns === 'number'         ? data.minValidTurns         : rec.minValid;

    const targetDurationMinutes = Math.round(Math.min(Math.max(rawDuration, rec.durationMin), rec.durationMax));
