...
import type { EvaluationHarness, NpcBehaviorHarness } from '../../../../shared/schema/scenarios';
...
function runRuleBasedEvaluation(input: EvaluationInput): TurnScore {
  ...
  empathy += empathyKeywords.filter(k => text.includes(k)).length * 12;
  ...
  logic += logicKeywords.filter(k => text.includes(k)).length * 12;
  if (sentences >= 2) logic += 5;
  ...
  ownership += ownershipKeywords.filter(k => text.includes(k)).length * 12;
  ...
  actionPlan += actionKeywords.filter(k => text.includes(k)).length * 10;
  if (len >= 150) actionPlan += 5;
...
function makeSkipScore(turnId: string, turnIndex: number): TurnScore {
  return {
    turnId, turnIndex,
    clarity: 0, empathy: 0, logic: 0, ownership: 0, actionPlan: 0,
    total: 0,
    evaluationMethod: 'rule',
