...
export class ElevenLabsService {
  ...
  private getVoiceSettings(emotion: string = '중립') {
    const emotionSettings = {
      '기쁨': { stability: 0.3, similarity_boost: 0.8, style: 0.6, use_speaker_boost: true },
      '슬픔': { stability: 0.8, similarity_boost: 0.5, style: 0.2, use_speaker_boost: false },
      '분노': { stability: 0.2, similarity_boost: 0.9, style: 0.8, use_speaker_boost: true },
      '놀람': { stability: 0.1, similarity_boost: 0.7, style: 0.9, use_speaker_boost: true },
      '중립': { stability: 0.5, similarity_boost: 0.7, style: 0.4, use_speaker_boost: true }
    };
  ...
  async generateSpeech(
    ...
    console.log(`🎤 ElevenLabs TTS 요청: ${scenarioId} (${gender}) - ${emotion}`);
