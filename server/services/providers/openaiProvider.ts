...
import { DEFAULT_DIMENSIONS, calculateWeightedOverallScore, checkMinValidTurns, filterVoiceNoise, calcEffectiveRatio, isVoiceMode, getScoreCap, calculateCompletionPenalty, applyEvidenceScoreCap, NO_EVIDENCE_SCORE_CAP, makeInsufficientEvidenceFallback, isValidEvidenceItem, analyzeBargeIn } from "../evaluationEngine";
...
JSON 형식으로 응답하세요: {"emotion": "감정", "reason": "감정을 느끼는 이유"}`;
  ...
  private async _generateFeedbackInner(
    ...
    // 시나리오별 목표 설정 추출
    const scenarioObj = typeof scenario === 'string' ? ({} as RoleplayScenario) : scenario;
    const scenarioTargetTurns: number | undefined = typeof scenarioObj.targetTurns === 'number' ? scenarioObj.targetTurns : undefined;
    const scenarioTargetDurationMinutes: number | undefined = typeof scenarioObj.targetDurationMinutes === 'number' ? scenarioObj.targetDurationMinutes : undefined;
    const scenarioMinValidTurns: number | undefined = typeof scenarioObj.minValidTurns === 'number' ? scenarioObj.minValidTurns : undefined;
    ...
    const msgTimestamps = messages
      ...
      .filter(t => t > 0);
    const actualDurationSeconds: number | undefined = msgTimestamps.length >= 2
      ? Math.floor((Math.max(...msgTimestamps) - Math.min(...msgTimestamps)) / 1000)
      : undefined;

    // 최소 유효 턴 수 확인 — 미달 시 평가 불가 반환
    const voiceMode = isVoiceMode(conversation);
    const userMessages = voiceMode
      ? filterVoiceNoise(messages.filter(m => m.sender === 'user'))
      : messages.filter(m => m.sender === 'user');
    const minValid = scenarioMinValidTurns ?? 4;
    if (!checkMinValidTurns(userMessages, minValid)) {
      const insufficient: DetailedFeedback = {
        overallScore: 0,
        scores: [],
        strengths: [],
        improvements: [],
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const bargeInSummaryText = bargeInAnalysis.count > 0
          ? `${bargeInAnalysis.count}회 발생 (적극적 참여: ${bargeInAnalysis.positiveCount}회 / 경청 부족: ${bargeInAnalysis.negativeCount}회 / 중립: ${bargeInAnalysis.neutralCount}회, AI 발화 대비 비율: ${Math.round(bargeInAnalysis.rate * 100)}%)\n${bargeInAnalysis.events.slice(0, 3).map((e, i) => `  [${i + 1}] [${e.assessment === 'positive' ? '✅ 적극적 참여' : e.assessment === 'negative' ? '❌ 경청 부족' : '➖ 중립'}] AI: "${e.aiMessage}" → 사용자: "${e.userMessage}" — ${e.reason}`).join('\n')}\n→ 시스템 요약: ${bargeInAnalysis.summary}\n→ 순 점수 조정: ${bargeInAnalysis.netScoreAdjustment >= 0 ? '+' : ''}${bargeInAnalysis.netScoreAdjustment}점 (시스템이 별도 적용)`
          : undefined;

        const feedbackPrompt = this.buildFeedbackPrompt(conversationText, messages, persona, evaluationCriteria, language, bargeInSummaryText);
        ...
        const feedback = this.parseFeedbackResponse(feedbackData, evaluationCriteria, userMessages, voiceMode, scenarioTargetTurns, actualDurationSeconds, scenarioTargetDurationMinutes, messages);
  ...
  private buildFeedbackPrompt(conversationText: string, messages: ConversationMessage[], persona: ScenarioPersona, evaluationCriteria?: EvaluationCriteriaWithDimensions, language: SupportedLanguage = 'ko', bargeInSummary?: string): string {
...
${bargeInSummary ? `\n## 🎤 말 끊기(Barge-in) 분석:\n${bargeInSummary}\n→ 이 말 끊기 패턴이 경청·공감 역량 평가에 미치는 영향을 반영하세요.` : ''}
...
JSON 형식으로 응답:
{
  ...
  private parseFeedbackResponse(
    ...
    // 완성도(effectiveRatio) 계산 — 시나리오 목표 기반 정규화 (턴 수 + 내용량 + 시간 비율의 최댓값)
    const effectiveRatio = userMessages && userMessages.length > 0
      ? calcEffectiveRatio(userMessages, voiceMode ?? false, scenarioTargetTurns, actualDurationSeconds, scenarioTargetDurationMinutes)
      : 1.0;

    // 공용 함수로 점수 캡 및 패널티 계산
    const scoreCap = getScoreCap(effectiveRatio);
    const completionPenalty = calculateCompletionPenalty(effectiveRatio, voiceMode ?? false);

    const rawScores: Record<string, number> = {};
    for (const dim of dimensions) {
      const rawScore = feedbackData.scores?.[dim.key];
      const maxAllowed = scoreCap !== null ? Math.min(scoreCap, dim.maxScore) : dim.maxScore;
    ...
    // 말 끊기(Barge-in) 분석 — 전체 메시지 목록이 있을 때만 수행
    const bargeInAnalysis = analyzeBargeIn(allMessages ?? []);
    ...
    const overallScore = Math.max(0, baseOverall - completionPenalty + bargeInAnalysis.netScoreAdjustment);

    if (bargeInAnalysis.count > 0) {
      console.log(`🎤 OpenAI 말 끊기 분석: ${bargeInAnalysis.count}회 (긍정 ${bargeInAnalysis.positiveCount} / 부정 ${bargeInAnalysis.negativeCount} / 중립 ${bargeInAnalysis.neutralCount}), 점수 조정: ${bargeInAnalysis.netScoreAdjustment >= 0 ? '+' : ''}${bargeInAnalysis.netScoreAdjustment}점`);
    }

    let improvements: string[] = feedbackData.improvements || ["더 구체적인 표현", "감정 교감 증진", "논리적 구조화"];
    if (completionPenalty > 0) {
      improvements = ["대화를 더 충분히 진행하면 더 정확한 역량 평가가 가능합니다", ...improvements];
    }
    if (bargeInAnalysis.events.filter(e => e.assessment === 'negative').length > 0) {
      improvements = ["상대방의 발화가 끝날 때까지 기다리는 경청 습관을 기르세요", ...improvements];
    }
    ...
    return {
      ...
      bargeInAnalysis: bargeInAnalysis.count > 0 ? bargeInAnalysis : undefined,
      scoreAdjustments: {
        // baseScore is the pre-evidence-cap weighted score so that:
        // baseScore - evidencePenalty - completionPenalty + bargeInAdj = finalScore
