...
import type { AIServiceInterface, ScenarioPersona, AIServiceConfig, EvaluationCriteriaWithDimensions, RoleplayScenario, SupportedLanguage } from "../aiService";
...
const EMOTION_LABELS: Record<SupportedLanguage, {
  sad: string; happy: string; angry: string; surprised: string; neutral: string;
}> = {
  ko: { sad: '슬픔', happy: '기쁨', angry: '분노', surprised: '놀람', neutral: '중립' },
  en: { sad: 'sad', happy: 'happy', angry: 'angry', surprised: 'surprised', neutral: 'neutral' },
  ja: { sad: '悲しみ', happy: '喜び', angry: '怒り', surprised: '驚き', neutral: '中立' },
  zh: { sad: '悲伤', happy: '喜悦', angry: '愤怒', surprised: '惊讶', neutral: '中立' },
};

const EMOTION_REASONS: Record<SupportedLanguage, {
  sad: string; happy: string; angry: string; surprised: string; neutral: (name: string) => string;
}> = {
  ko: {
    sad: '사과나 어려움을 표현하는 상황',
    happy: '긍정적이고 만족스러운 상황',
    angry: '문제 상황이나 부정적 상황에 대한 반응',
    surprised: '예상치 못한 상황이나 질문에 대한 반응',
    neutral: (name) => `${name}의 평상시 업무적 대화`,
  },
  en: {
    sad: 'Expressing an apology or difficulty',
    happy: 'A positive and satisfying situation',
    angry: 'A reaction to a problem or negative situation',
    surprised: 'A reaction to an unexpected situation or question',
    neutral: (name) => `${name}'s routine work conversation`,
  },
  ja: {
    sad: '謝罪や困難を表現している状況',
    happy: 'ポジティブで満足のいく状況',
    angry: '問題や否定的な状況への反応',
    surprised: '予期しない状況や質問への反応',
    neutral: (name) => `${name}の通常の業務的会話`,
  },
  zh: {
    sad: '表达歉意或困难的情况',
    happy: '积极且令人满意的情况',
    angry: '对问题或消极情况的反应',
    surprised: '对意外情况或问题的反应',
    neutral: (name) => `${name}的日常工作对话`,
  },
};

const EMOTION_KEYWORDS: Record<SupportedLanguage, {
  sad: string[]; happy: string[]; angry: string[]; surprised: string[];
}> = {
  ko: {
    sad: ['죄송', '미안', '어려워'],
    happy: ['좋', '감사', '잘'],
  ...
  en: {
    sad: ['sorry', 'difficult', 'unfortunately', 'regret', 'apologize'],
    happy: ['great', 'thank', 'excellent', 'good', 'appreciate', 'well done'],
    angry: ['problem', 'issue', 'cannot', "can't", 'wrong', 'unacceptable'],
    surprised: ['?', 'really', 'unexpected', 'interesting', 'wow'],
  },
  ja: {
    sad: ['すみません', '申し訳', '難し', '残念'],
    happy: ['ありがとう', '良い', 'よかった', '素晴らしい'],
    angry: ['問題', '困る', 'できない', 'だめ'],
    surprised: ['?', '本当', 'どうして', '意外'],
  },
  zh: {
    sad: ['对不起', '抱歉', '遗憾', '困难'],
    happy: ['谢谢', '很好', '优秀', '满意'],
...
export class CustomProvider implements AIServiceInterface {
  ...
  async generateResponse(
    ...
    const lang: SupportedLanguage = (language as SupportedLanguage) || 'ko';
    try {
...
3. ${LANGUAGE_INSTRUCTIONS[language as SupportedLanguage] || LANGUAGE_INSTRUCTIONS['ko']}
...
사용자의 새 메시지에 ${persona.name}로서 응답하세요.`;
      ...
      // 감정 분석
      const labels = EMOTION_LABELS[lang] || EMOTION_LABELS['ko'];
      let emotion = labels.neutral;
      let emotionReason = EMOTION_REASONS[lang]?.neutral(persona.name) || EMOTION_REASONS['ko'].neutral(persona.name);

      if (userMessage) {
        const emotionAnalysis = await this.analyzeEmotion(content, persona, userMessage, lang);
      ...
      const errorLabels = EMOTION_LABELS[lang] || EMOTION_LABELS['ko'];
      const errorReasons = EMOTION_REASONS[lang] || EMOTION_REASONS['ko'];
      return { 
        content: fallbackContent, 
        emotion: errorLabels.neutral, 
        emotionReason: errorReasons.neutral(persona.name)
      };
    }
  }
  ...
  private async analyzeEmotion(
    ...
    if (this.config.apiFormat === 'custom' || this.config.apiKey === 'test-key') {
      ...
      return this.analyzeEmotionByRules(response, persona, userMessage, language);
    }

    // OpenAI 호환 API만 실제 감정 분석 시도
    try {
      const lb = EMOTION_LABELS[language] || EMOTION_LABELS['ko'];
      const emotionPromptByLang: Record<SupportedLanguage, string> = {
        ko: `다음 대화에서 ${persona.name}의 감정 상태를 분석하세요.\n\n${persona.name}의 성격: ${persona.personality}\n사용자 메시지: "${userMessage}"\n${persona.name}의 응답: "${response}"\n\n다음 중 하나의 감정으로 분류하고 이유를 설명하세요:\n- ${lb.happy}: 만족, 즐거움, 긍정적 반응\n- ${lb.sad}: 실망, 우울, 부정적 감정\n- ${lb.angry}: 화남, 짜증, 불만\n- ${lb.surprised}: 의외, 당황, 예상치 못한 반응\n- ${lb.neutral}: 평상심, 차분함, 일반적 상태\n\nJSON 형식으로 응답하세요: {"emotion": "감정", "reason": "감정을 느끼는 이유"}`,
        en: `Analyze the emotional state of ${persona.name} in the following exchange.\n\n${persona.name}'s personality: ${persona.personality}\nUser message: "${userMessage}"\n${persona.name}'s response: "${response}"\n\nClassify into one of the following emotions and explain the reason:\n- ${lb.happy}: satisfaction, joy, positive reaction\n- ${lb.sad}: disappointment, gloom, negative feeling\n- ${lb.angry}: annoyance, irritation, dissatisfaction\n- ${lb.surprised}: unexpected, flustered, unanticipated reaction\n- ${lb.neutral}: calm, composed, general state\n\nRespond in JSON: {"emotion": "emotion", "reason": "reason for the emotion"}`,
        ja: `次の会話における${persona.name}の感情状態を分析してください。\n\n${persona.name}の性格: ${persona.personality}\nユーザーのメッセージ: "${userMessage}"\n${persona.name}の返答: "${response}"\n\n以下のいずれかの感情に分類し、理由を説明してください:\n- ${lb.happy}: 満足、喜び、ポジティブな反応\n- ${lb.sad}: 失望、憂鬱、ネガティブな感情\n- ${lb.angry}: 怒り、苛立ち、不満\n- ${lb.surprised}: 意外、戸惑い、予期しない反応\n- ${lb.neutral}: 平常心、落ち着き、通常の状態\n\nJSON形式で応答してください: {"emotion": "感情", "reason": "感情の理由"}`,
        zh: `请分析以下对话中${persona.name}的情绪状态。\n\n${persona.name}的性格: ${persona.personality}\n用户消息: "${userMessage}"\n${persona.name}的回复: "${response}"\n\n请从以下情绪中选择一种并解释原因:\n- ${lb.happy}: 满意、喜悦、积极反应\n- ${lb.sad}: 失望、忧郁、消极情绪\n- ${lb.angry}: 愤怒、烦躁、不满\n- ${lb.surprised}: 意外、困惑、出乎意料的反应\n- ${lb.neutral}: 平静、从容、一般状态\n\n请以JSON格式回复: {"emotion": "情绪", "reason": "情绪原因"}`,
      };
      const emotionPrompt: string = emotionPromptByLang[language] || emotionPromptByLang['ko'];
      ...
      const emotionText = data.choices?.[0]?.message?.content || `{"emotion": "${lb.neutral}", "reason": ""}`;
      
      const emotionData = JSON.parse(emotionText);
      return {
        emotion: emotionData.emotion || lb.neutral,
        reason: emotionData.reason || EMOTION_REASONS[language]?.neutral(persona.name) || ''
      };
    } catch (error) {
  ...
  private analyzeEmotionByRules(
    ...
    const lb = EMOTION_LABELS[language] || EMOTION_LABELS['ko'];
    const rs = EMOTION_REASONS[language] || EMOTION_REASONS['ko'];
    const kw = EMOTION_KEYWORDS[language] || EMOTION_KEYWORDS['ko'];

    const matchesAny = (text: string, keywords: string[]) =>
      keywords.some(k => text.includes(k.toLowerCase()));

    if (matchesAny(responseText, kw.sad)) {
      return { emotion: lb.sad, reason: rs.sad };
    }

    if (matchesAny(responseText, kw.happy)) {
      return { emotion: lb.happy, reason: rs.happy };
    }

    if (matchesAny(responseText, kw.angry) || matchesAny(userText, kw.angry)) {
      return { emotion: lb.angry, reason: rs.angry };
    }

    if (matchesAny(responseText, kw.surprised)) {
      return { emotion: lb.surprised, reason: rs.surprised };
    }

    return { emotion: lb.neutral, reason: rs.neutral(persona.name) };
  }

  private buildFeedbackPrompt(conversationText: string, persona: ScenarioPersona, evaluationCriteria?: EvaluationCriteriaWithDimensions, language?: SupportedLanguage): string {
...
**중요**: ${LANGUAGE_INSTRUCTIONS[language as SupportedLanguage] || LANGUAGE_INSTRUCTIONS['ko']}
...
JSON 형식으로 응답:
{
  ...
  async generateFeedback(
    ...
    return feedbackSemaphore.run(() => this._generateFeedbackInner(conversationText, persona, evaluationCriteria, language));
  }
