...
3. 전략적 통찰력 (1-5점): 피평가자가 전략적 사고를 잘 보여주는가?
...
${isIncomplete
  ? voiceMode
    ? `\n📉 음성 대화 완성도 부족: 대화량이 부족합니다 — 근거가 없는 역량은 1-2점, 부분적 근거가 있으면 최대 3점, 4점 이상은 명확한 긍정 근거가 있을 때만 허용하세요.
→ 시스템이 완성도 패널티를 별도 적용합니다 (음성 모드 상한 15점). 참여도·적극성 관련 영역은 낮게 평가하세요.\n`
    : `\n📉 대화 완성도 부족: 대화량이 부족합니다 — 근거가 없는 역량은 1-2점, 부분적 근거가 있으면 최대 3점, 4점 이상은 명확한 긍정 근거가 있을 때만 허용하세요.
...
- 피평가자의 각 발화를 분석하여 어떤 영역과 관련되는지 매핑하세요. 관련 발화가 없는 영역은 낮은 점수를 부여합니다
...
${isIncomplete ? `- **⛔ 대화량 부족 시 추가 원칙**: 근거가 없는 역량은 반드시 1-2점으로 평가하세요. 부분적 근거가 있는 역량은 최대 3점까지만 허용합니다. 4점 이상은 해당 역량을 명확하게 보여주는 긍정적 발화가 존재할 때만 가능합니다. 점수가 모두 비슷하거나 3-4점대에 몰리지 않도록 반드시 차등을 두어야 합니다. 예시 분포: 근거 없음→1점, 미흡→2점, 부분 근거→3점, 명확 근거→4점, 탁월→5점.` : ''}
